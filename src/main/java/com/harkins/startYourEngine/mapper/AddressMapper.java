...
public interface AddressMapper {
    ...
    default String buildFullAddress(SaveAndUpdateAddressRequest request) {
        return String.format(
                "%s, %s, %s, %s", request.getStreet(), request.getCity(), request.getState(), request.getCountry());
    }

    default String buildFullAddress(Address address) {
        return String.format(
                "%s, %s, %s, %s", address.getStreet(), address.getCity(), address.getState(), address.getCountry());
    }
}
